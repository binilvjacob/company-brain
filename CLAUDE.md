...
- NO LangChain/LlamaIndex, NO hosted RAG SaaS, NO agent frameworks, NO fine-tuning, NO auth system (doc-level `visibility` tags + a role dropdown instead), NO real OAuth connectors (adapter interface + mock data + typed Notion stub). Post-v1 amendment: token-based live connectors are in scope — Telegram (webhook, BotFather token) and Slack (Events API, single-workspace token install — no OAuth flow) shipped over one shared core (connectors/common.py); OAuth-flow ones (Drive/Gmail/Notion, multi-workspace Slack) remain out.
